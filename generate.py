...
from model import LSTM_Writer, LSTM_ka_combi
...
def generate_unconditionally(cell_size=400, num_clusters=20, steps=800, random_state=700, \
    ...
    model = LSTM_Writer(cell_size, num_clusters)
    ...
    for i in range(steps):        
        last, weights, m1, m2, s1, s2, p, prev, prev2 = model(x, prev, prev2)
        
        # sample last stroke indicator
        prob_end = last.data[0][0][0]        
        ...
        mu = np.array([m1.data[0][0][sample_index], m2.data[0][0][sample_index]])
        v1 = s1.exp().data[0][0][sample_index]**2
        v2 = s2.exp().data[0][0][sample_index]**2
        c = p.data[0][0][sample_index]*s1.exp().data[0][0][sample_index]\
            *s2.exp().data[0][0][sample_index]
...
def generate_conditionally(text, cell_size=400, num_clusters=20, K=10, random_state=700, \
    ...
    model = LSTM_ka_combi(len(text), len(char_to_code)+1, cell_size, num_clusters, K)
    model.load_state_dict(torch.load(state_dict_file)['model'])
    
    onehots = np.zeros((len(text), len(char_to_code)+1))
    for i in range(len(text)):
        try:
            onehots[i][char_to_code[text[i]]] = 1
        except:
            onehots[i][-1] = 1
    ...
    while not stop:    
        ...
        last, weights, m1, m2, s1, s2, rho, w_old, kappa_old, prev, prev2, old_phi = outputs
        ...
        v1 = (s1).exp().data[0][0][sample_index]**2
        v2 = (s2).exp().data[0][0][sample_index]**2
        c = rho.data[0][0][sample_index]*s1.exp().data[0][0][sample_index]\
            *s2.exp().data[0][0][sample_index]
        cov = np.array([[v1,c],[c,v2]])
        sample_point = np.random.multivariate_normal(mu, cov)
