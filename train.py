...
def rand_write_train(args, train_loader, validation_loader):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, (data, masks, onehots, text_lens) in enumerate(train_loader):
            ...
            end, weights, m1, m2, s1, s2, rho , prev, prev2 = outputs
            
            # supervision
            data = data.narrow(1,1,args.timesteps)
            y = Variable(data, requires_grad=False)
            loss = -log_likelihood(end, weights, m1, m2, s1, s2, rho, y, masks)/torch.sum(masks)
            loss.backward()
...
def synthesis_train(args, train_loader, validation_loader):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, (data, masks, onehots, text_lens) in enumerate(train_loader):
            ...
            end, weights, m1, m2, s1, s2, rho, w, kappa, prev, prev2, old_phi = outputs
            data = data.narrow(1,1,args.timesteps)
            y = Variable(data, requires_grad=False)
            loss = -log_likelihood(end, weights, m1, m2, s1, s2, rho, y, masks)/torch.sum(masks)
            loss.backward()
...
# training objective
def log_likelihood(end, weights, m1, m2, s1, s2, rho, \
                    y, masks):
    # targets
    y_0 = y.narrow(-1,0,1)
    y_1 = y.narrow(-1,1,1)
    ...
    z = (y_1 - m1)**2/(s1.exp()**2)\
        + ((y_2 - m2)**2/(s2.exp()**2)) \
        - 2*rho*(y_1-m1)*(y_2-m2)/((s1 + s2).exp())
    mog_lik1 =  pi_term -s1 - s2 - 0.5*((1-rho**2).log())
